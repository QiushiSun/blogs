...
$(function () { //获取处理友链数据
    $.getJSON("../json_data/gallery.json", function (data) {
        // var data0 = data[0];
        $('.links-content').html("");
        // 随机排序过滤失效的
        ...
        $('.links-content').append("<div class='friend-title-item'><br>Selected Honors & Awards & Publications<br><br><hr></div>");
        $.each(data, function (i, e) {
            var html = "<div class=\"friend-card-item\">";
            if (e.src == undefined) {
                html += "    <img class=\"ava\" src=\"/img/links/nopic.jpg\" title=\"Default Pics\">";
            } else {
                html += "    <img class=\"ava\" src=\"" + e.src + "\">";
            }
            html +=
                "<div class='text-desc' title=\""+e.desc+"\">    Name：<a href=\"" + e.url + "\" target=\"_blank\">" + e.name + "</a>" +
                "    <br>Time：" + e.date +
